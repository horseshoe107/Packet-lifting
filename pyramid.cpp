...
#include "base.h"
#include "dwtnode.h"
double mpegB[] = {2,0,-4,-3,5,19,26,19,5,-3,-4,0,2}; // 13-tap downsampler
const int mpegN = 6;
double h264[] = {1,0,-5,0,20,32,20,0,-5,0,1}; // 11-tap upsampler
const int h264N = 5;
...
void dwtnode::downsample_lift(bool analysis)
{
  ...
  for (int y=0;y<subbands[0]->h;y++)
    for (int x=0;x<w;x++)
      tmp->pixels[2*y*w+x] = filt(&mpegB[mpegN],2*y*w+x,0,mpegN,vertical,true)/64;
  // horizontal filter; skip every 2nd row and column
  for (int y=0;y<subbands[0]->h;y++)
    for (int x=0;x<subbands[0]->w;x++)
      subbands[0]->pixels[y*subbands[0]->w+x]
        += sign*tmp->filt(&mpegB[mpegN],2*y*w+2*x,0,mpegN,horizontal,true)/64;
  delete tmp;
  return;
}
